(function ($) {
    ...
    function addActiveClass(pageSlug) {
        ...
        menuLinks.each(function () {
            ...
            if (!menuItemPath) return; // pula se não tiver href
            ...
            $.ajax({
                ...
                success: function (response) {
                    switch (response.status) {
                        case 'success':
                            Swal.fire({
                                title: 'Deletado!', text: response.message, icon: "success", showConfirmButton: false, customClass: {
                                    title: 'text-dark'
                                }
                            });
                            setTimeout(() => {
                        ...
                        default:
                            Swal.fire({
                                title: "Erro!", text: response.message, icon: "error", confirmButtonColor: "#4a4fea", customClass: {
                                    title: 'text-dark'
                                }
                            });
                            break;
...
fileInputPreview.forEach(function (fileInputPreviewI) {
    ...
    const removeFile = fileInputPreviewI.querySelector('.removeFile');

    fileInput.addEventListener('change', function (event) {
        const files = Array.from(event.target.files);

        const existingPreview = fileInputPreviewI.parentNode.querySelector('.previewFiles');
        if (existingPreview) existingPreview.remove();

        if (files.length > 1) {
            const previewFiles = document.createElement('div');
            previewFiles.className = 'previewFiles';
    ...
    if (previewFileImg.getAttribute('src') != '') {
        fileInputPreviewI.classList.add('preview');
    }
    ...
    removeFile.addEventListener('click', function (event) {
        ...
        fileInput.value = '';
        previewFileImg.src = '';
        previewFileImg.style.display = 'none';
        removeFile.style.display = 'none';
        labelInput.classList.remove('preview');
        fileInputPreviewI.classList.remove('preview');
    });
});
